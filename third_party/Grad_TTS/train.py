...
if __name__ == "__main__":
    ...
    model = GradTTS(nsymbols, params.embedding_size, n_enc_channels, filter_channels, filter_channels_dp,
                    n_heads, n_enc_layers, enc_kernel, enc_dropout, window_size, 
                    n_feats, dec_dim, beta_min, beta_max, pe_scale).cuda()

    if len(params.pretrain) > 0:
        model.load_state_dict(torch.load(params.pretrain, map_location=lambda loc, storage: loc))
        print('Model has been loaded from checkpoint: {}'.format(params.pretrain))
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        with torch.no_grad():
            for i, item in enumerate(test_batch):
                ...
                emb = item['embedding'].cuda().unsqueeze(0)
                y_enc, y_dec, attn = model(x, x_lengths, n_timesteps=50, emb=emb)
