...
def plot_projections(embeds, speakers, out_path, model_name='current_model', ax=None, colors=None, markers=None, legend=True,
                     title="", **kwargs):
    if ax is None:
    ...
    ax.set_yticks([])
    ax.set_aspect("equal")
    plt.savefig(os.path.join(out_path, model_name+'.png'))
...
def createParser():
    ...
    parser.add_argument('-m', '--path_to_model')
...
if __name__ == '__main__':
    ...
    path_to_data = namespace.path_to_data
    model_path = namespace.path_to_model
    path_to_save = namespace.path_to_out_dir

    model = Path(model_path)

# Path('C:/Users/zador/Desktop/RTVC/encoder/saved_models/tanh_run.pt')
...
## Gather the wavs
    wav_fpaths = list(Path(path_to_data).glob("**/*.wav"))
    speakers = list(map(lambda wav_fpath: wav_fpath.parent.stem, wav_fpaths))
    wavs = np.array(list(map(encoder.preprocess_wav, tqdm(wav_fpaths, "Preprocessing wavs", len(wav_fpaths)))))
...
## Project the embeddings in 2D space
    plot_projections(utterance_embeds, speakers, out_path=path_to_save, model_name=model_name,
                     title=f"Embedding projections from {model_name}")
    plt.show()
